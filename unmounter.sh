#!/bin/sh

# From Lukesmith's LUKS Scripts

# Unmount USB drives or Android phones. Replaces the older `dmenuumount`. Fewer
# prompt and also de-decrypts LUKS drives that are unmounted.

set -e

mounteddroids="$(grep simple-mtpfs /etc/mtab | awk '{print $2}' | sed 's/\\040/ /g; s/\\0402$//')"
lsblkoutput="$(lsblk -nrpo "name,type,size,mountpoint")"
mounteddrives="$(echo "$lsblkoutput" | awk '($2=="part"||$2="crypt")&&$4!~/\/boot|\/home$|SWAP|\/media\/external_drive/&&length($4)>1{printf "💾%s (%s)\n",$4,$3}')"

allunmountable="$(echo "$mounteddroids
$mounteddrives" | sed "/^$/d;s/ *$//")"
test -n "$allunmountable"

chosen="$(echo "$allunmountable" | dmenu -i -p "Unmount which drive?")"

case "$chosen" in
    /media/[0-9]*)  ;;
        *) chosen="${chosen%% *}" ;;
esac

echo "$chosen"

test -n "$chosen"

sudo -A umount "/${chosen#*/}"
sudo -A rmdir "/${chosen#*/}"

notify-send "Device unmounted." "$chosen has been unmounted."
